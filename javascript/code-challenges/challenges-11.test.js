...
let findMaleAndFemale = (data) => {
  ...
  let X = '';
  data.map((item, idx) => { if (idx === data.length - 1) {
 X=X+item.name;
    }
...
let findShortest = (data) => {
  ...
  let minmum = data[0];
data.map(item => {
 if(parseInt(item.height) < parseInt(minmum.height)){
minmum = item;
    }
  });
  return minmum.name;
